 ...
 * @return {Array.<hello.notes.Note>} An array containing the resulting
 *     instances.
 */
hello.notes.makeNotes = function(data, noteContainer) {
  var notes = [];
  for (var i = 0; i < data.length; i++) {
    var note = 
      new hello.notes.Note(data[i].title, data[i].content, noteContainer);
    notes.push(note);
    note.makeNoteDom();
  }
  return notes;
 ...
 * @constructor
 */
hello.notes.Note = function(title, content, noteContainer) {
  this.title = title;
 ...
 */
hello.notes.Note.prototype.makeNoteDom = function() {
